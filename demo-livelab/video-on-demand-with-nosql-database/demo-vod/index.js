...
async function createStreamHelper(input) {
  input.shows= [];
  let res = await client.putIfAbsent(TABLE_NAME, {
        info: input
  });
...
async function updateStreamHelper(id, input) {
  ...
  let statement = `DECLARE $v_show JSON; $v_id INTEGER; UPDATE ${TABLE_NAME} p ADD p.info.shows $v_show WHERE id=$v_id RETURNING *`;
  ...
  let res = await client.query(preparedStmt);
  return (res.rows[0]);
}
...
async function deleteStreamHelper(id) {
  let res = await client.delete(TABLE_NAME, {
        id: id
  });
  let delStream = {id: id, info: null};
  return delStream;
}
