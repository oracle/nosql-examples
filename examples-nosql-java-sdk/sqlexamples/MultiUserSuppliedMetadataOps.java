...
public class MultiUserSuppliedMetadataOps {
    /* Name of your table */
    final static String tableName = "Emp_Address";
    static NoSQLHandle handle;

    final static String emp1 =
        "{ \"id\": 1, \"address_line1\": \"10 Red Street\", " +
            "\"address_line2\": \"Apt 3\", \"pin\": 1234567 }";
    final static String emp2 =
        "{ \"id\": 2, \"address_line1\": \"2 Green Street\", " +
            "\"address_line2\": \"Suite 9\", \"pin\": 1234567 }";
    final static String emp3 =
        "{ \"id\": 3, \"address_line1\": \"5 Blue Ave\", " +
            "\"address_line2\": \"Floor 2\", \"pin\": 1234567 }";

    final static String rm1 =
        "{\"modified_by\" : \"Shreya Sharma\",\"joined_in\" : " +
            "\"January 2012\",\"update_reason\" : \"Change in address\"}";
    final static String rm2 =
        "{\"modified_by\" : \"Slvia Sanz\",\"data_source\" : " +
            "\"individual update\"}";
    final static String rm3 =
        "{\"modified_by\" : \"Raul Daniels\",\"joined_in\" : " +
            "\"September 2023\"}";
    final static String rm4 =
        "{\"modified_by\" : \"Shreya Sharma\",\"joined_in\" : " +
            "\"May 2004\",\"update_reason\":\"Internal job transfer\"}";
    ...
    public static void main(String[] args)
         ...
         * NoSQL Database.
         * Leave the lines commented if you are using NoSQL Database Cloud
         * Service
        ...
        try {
            ...
            /*
             * The below function call is to write multiple rows with row
             * metadata
             */
            writeMultipleRows(handle, (MapValue) newvalue, (MapValue) newvalue1,
                (MapValue) newvalue2, rm1, rm2, rm3);

            writeRows(handle, 4, "9 Yellow Boulevard",
                "Apt 3", 87654321, rm4);

            /*
             * The below function call is to delete multiple rows with row
             * metadata
             */
    ...
    private static NoSQLHandle generateNoSQLHandleonPrem(
    ...
    private static NoSQLHandle generateNoSQLHandleCloud(String region,
        String compId)
        throws Exception {
    ...
    /* Add a row of data */
    private static void writeRows(NoSQLHandle handle, int idval, String add1,
        String aad2, int pinval, String rm4)
        throws Exception {
    ...
    private static void writeMultipleRows(NoSQLHandle handle, MapValue val1,
        MapValue val2, MapValue val3, String rm1, String rm2, String rm3) {
        PutRequest put1 = new PutRequest()
            .setValue(val1)
            .setTableName(tableName)
            .setLastWriteMetadata(rm1);
        PutRequest put2 = new PutRequest()
            .setValue(val2)
            .setTableName(tableName)
            .setLastWriteMetadata(rm2);
        PutRequest put3 = new PutRequest()
            .setValue(val3)
            .setTableName(tableName)
            .setLastWriteMetadata(rm3);
        ...
        if (wmResult.getSuccess()) {
            System.out.println(
                "Successfully inserted all 3 rows with rowMetadata.");
        } else {
            System.out.println(
                "WriteMultiple failed. Some or all operations may not have " +
                    "succeeded.");
        }
    }

    /* Delete multiple rows and set row metadata */
    private static void delMulRows(NoSQLHandle handle, int pinval)
        throws Exception {
        ...
        MultiDeleteRequest multiDelRequest = new MultiDeleteRequest()
            ...
            .setLastWriteMetadata(
                "{\"deletedBy\": \"Hannah Lee\",\"reason\":\"Employee no " +
                    "longer works in the company\"}");
