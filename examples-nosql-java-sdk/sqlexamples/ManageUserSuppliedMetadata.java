...
public class ManageUserSuppliedMetadata {
    /* Name of your table */
    ...
    final static String rm1 =
        "{\"modified_by\" : \"John Doe\",\"reviewed_in\" : " +
    ...
    public static void main(String[] args)
        ...
        /*
         * UNCOMMENT the 2 lines of code below if you are using onPremise
         ...
         *      String kvstore_endpoint ="http://<hostname>:8080";
        ...
        try {
            ...
            /*
             * The below function call is to fetch row-metadata using GET API
             * based on Primary key
             */
            readRowMetadata(handle, 1);

            /*
             * The below function call is to update the row and row-metadata
             * using a Query
             */
            ...
            /*
             * The below function call is to delete a row with row-metadata
             * based on Primary key
             */
    ...
    private static void writeRowWithMetadata(NoSQLHandle handle, MapValue value,
        String rowMetadata)
        throws Exception {
        PutRequest putRequest = new PutRequest()
            .setValue(value)
            .setTableName(tableName)
            .setLastWriteMetadata(rowMetadata);
    ...
    private static void readRowMetadata(NoSQLHandle handle, int acctId)
        throws Exception {
        ...
        if (rm != null) {
            System.out.println(
                "Row metadata for acct_Id " + acctId + ": " + rm);
        } else {
    ...
    private static void deleteRowWithMetadata(NoSQLHandle handle, int acctId)
        throws Exception {
        ...
        DeleteRequest delRequest = new DeleteRequest()
            .setKey(key)
            .setTableName(tableName)
            /* Ask to return the deleted row and its metadata */
            .setReturnRow(true)
            /* Metadata attached to deletion */
            .setLastWriteMetadata(
                "{\"deletedBy\": \"Hannah Lee\",\"reason\":" +
                    "\"User requested account removal\"}");

        DeleteResult del = handle.delete(delRequest);
        String rm = del.getExistingLastWriteMetadata();

        if (rm != null) {
            System.out.println(
                "Deleted acct_Id " + acctId + " previous row metadata: " + rm);
        } else {
            System.out.println(
                "Row with acct_Id " + acctId + " did not exist.");
        }
    }

    /* Update query and row metadata using QueryRequest API */
    private static void updateRowViaQuery(NoSQLHandle handle)
        throws Exception {
        /*
         * UPDATE acct_Id 1 with new profile_name and metadata using SQL
         * query
         */
        String updateQuery = "UPDATE " + tableName +
            " SET profile_name = 'Amelia Pure' WHERE acct_Id = 1";
        String rowMetadata =
            "{\"modifiedBy\": \"Priya Kaul\", \"updateReason\": \"full name\"}";
        ...
        handle.query(qr);
        System.out.println(
            "Updated acct_Id 1 via query with row metadata: " + rowMetadata);
    }

    /* Fetch row metadata via query function using QueryRequest API */
    private static void readAllRowMetadataViaQuery(NoSQLHandle handle)
        throws Exception {
        /* SELECT query to retrieve only row metadata for all rows */
        String query =
            "SELECT row_metadata($t) as rmt FROM " + tableName + " $t";
