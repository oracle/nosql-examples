...
async function doMultiDataOps() {
   ...
   try {
      ...
      * Cloud service. Leave the line commented if you are using onPrem database
      ...
      let putResult = await handle.put(TABLE_NAME, JSON.parse(add1));
      let putResult1 = await handle.put(TABLE_NAME, JSON.parse(add2));
      let putResult2 = await handle.put(TABLE_NAME, JSON.parse(add3));
      let putResult3 = await handle.put(TABLE_NAME, JSON.parse(add4));
...
async function fetchRowCnt(handle,querystmt) {
   const opt = {};
   try {
      do {
