...
interface ParentTableInt {
   ticketNo: String;
   confNo: String;
}

interface ChildTableInt {
   id: Long;
   tagNum: Long;
   routing: String;
   ...
   lastSeenTimeGmt: TIMESTAMP;
   bagArrivalDate: TIMESTAMP;
}

interface DesctableInt {
   flightNo: String;
...
async function dotablejoins() {
   ...
   try {
      ...
      * Cloud service. Leave the line commented if you are using onPrem database*/
      ...
      let putResult = await handle.put<ParentTableInt>(table_name, JSON.parse(data1));
      console.log("Wrote records in ticket table");
      let putResult1 = await handle.put<ChildTableInt>(childtable_name, JSON.parse(data2));
...
/*creates a table*/
async function createTable(handle: NoSQLClient, query_stmt: string, reg_table: boolean, table_name: string): Promise<void> {
   let res;
   if(reg_table==true){
